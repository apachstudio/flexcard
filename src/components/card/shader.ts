...
half4 main(float2 fragCoord) {
  ...
  // header. The base curvature 1.35 (how much the "sphere" bulges across the
  // card) was tuned by eye against the Figma instances at Zoom = 29%, so the
  // Zoom slider scales it inversely around that anchor: zooming in enlarges
  // the features (flatter direction), zooming out shrinks them.
  float curvature = 1.35 * (29.0 / max(zoomPercent, 1.0));
  float3 direction = normalize(float3(p.x * curvature, p.y * curvature, 1.0));
...
export type ShaderCardParams = {
  ...
  /** Ported from the Figma panel's "Zoom" slider (percent; 29 = shipped look). */
  zoom: number;
